const input = `eyr:2021 hgt:168cm hcl:#fffffd pid:180778832 byr:1923 ecl:amb iyr:2019 cid:241
...
const validators = {
    byr: (value) => inRange(parseInt(value, 10), 1920, 2002),
    iyr: (value) => inRange(parseInt(value, 10), 2010, 2020),
    eyr: (value) => inRange(parseInt(value, 10), 2020, 2030),
    hgt: (value) => {
        const unit = value.slice(-2);
        const v = parseInt(value.slice(0, -2), 10);
        switch (unit) {
            case 'cm':
                return inRange(v, 150, 193);
            case 'in':
                return inRange(v, 59, 76);
        }
    },
    ecl: (value) =>
        ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'].includes(value),
    hcl: (value) => value.match(/#\w{6}/),
    pid: (value) => value.length === 9,
};

const valid = input.split('\n\n').filter((passport) => {
    const fields = passport.match(/(byr|iyr|eyr|hgt|hcl|ecl|pid):(\S*)/g);
    return (
        fields.filter((f) => {
