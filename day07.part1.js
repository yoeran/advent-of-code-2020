...
const bagtypes = input.split('\n').map((rule) => {
    const [, type, contents] = rule.match(/(.*) bags contain (.*)\./);
    const contains = contents
        .split(', ')
        .map((sub) => {
            ...
            const [, amount, type] = sub.match(/(\d{1,2}) (.*) bag/);
            return { amount: parseInt(amount, 10), type };
        })
        .filter((n) => n);

    return { type, contains, parents: [] };
});

bagtypes.forEach((bagtype) => {
    bagtype.parents = bagtypes
        .filter((b) => b.contains.find((b2) => b2.type === bagtype.type))
        .map((b) => b.type);
    return bagtype;
});
...
const getParents = (bagType, parents) => {
    const bag = bagtypes.find((b) => b.type === bagType);
    if (bag.parents.length === 0) {
        return;
    }

    parents.push(...bag.parents);

    bag.parents.forEach((p) => {
        getParents(p, parents);
    });
};

const directbags = bagtypes.filter((b) =>
    b.contains.find((b2) => b2.type === 'shiny gold')
);
const parentbags = directbags
    .reduce((all, b) => {
        const parents = [b.type];
        getParents(b.type, parents);

        return [...all, ...parents];
    }, [])
    .filter(uniq);

console.log('Answer', parentbags.length);
