...
const getContent = (bagKey) => {
    const bag = bagtypes.find((b) => b.type === bagKey);
    const arr = [];

    arr.push(bagKey);

    bag.contains.forEach((sub) => {
        for (let i = 0; i < sub.amount; i++) {
            arr.push(...getContent(sub.type));
        }
    });
...
const childbags = getContent('shiny gold');

console.log('Answer', childbags.length - 1);
