...
const items = input.split('\n').map((line) => {
    const [, pos1, pos2, char, password] = line.match(/(\d+)-(\d+) (\w): (.*)/);
    const occ1 = password[parseInt(pos1, 10) - 1] === char;
    const occ2 = password[parseInt(pos2, 10) - 1] === char;
    const valid = (occ1 || occ2) && !(occ1 && occ2);

    return { pos1, pos2, char, password, occ1, occ2, valid };
});

console.log(items);
console.log('Answer', items.filter((i) => i.valid).length);
