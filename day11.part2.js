...
const getFirstSeat = (x, y, dx, dy) => {
    const ns = getCell(x + dx, y + dy);

    if (ns === '.') {
        return getFirstSeat(x + dx, y + dy, dx, dy);
    }
...
while (safeguard > 0) {
    ...
    for (let x = 0; x < cols; x++) {
        for (let y = 0; y < rows; y++) {
            ...
            const tl = getFirstSeat(x, y, -1, -1);
            const t = getFirstSeat(x, y, 0, -1);
            const tr = getFirstSeat(x, y, 1, -1);
            const l = getFirstSeat(x, y, -1, 0);
            const r = getFirstSeat(x, y, 1, 0);
            const b = getFirstSeat(x, y, 0, 1);
            const lb = getFirstSeat(x, y, -1, 1);
            const rb = getFirstSeat(x, y, 1, 1);
            const adjacentOccupied = [tl, t, tr, r, l, b, lb, rb].filter(
                (c) => c === '#'
            ).length;
...
console.log('Answer', grid.join('').match(/#/g).length);
